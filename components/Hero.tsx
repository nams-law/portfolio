...
export const Hero: React.FC = () => {
  const { t, scrollToSection, locale } = useLanguage();
    ...
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-stone-50">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full relative z-10">
        <div className="max-w-2xl">
          <div 
            className={`inline-flex items-center gap-2 px-4 py-2 bg-stone-900 text-stone-100 text-[11px] font-bold mb-8 rounded-full shadow-lg ${isNepali ? 'lang-ne' : 'uppercase tracking-[0.2em]'}`}
          >
            <CheckCircle2 className="w-3.5 h-3.5 text-green-400" />
            {t(UI_TEXT.freelanceStatus)}
          </div>
          
          <h1 className={`text-5xl md:text-7xl font-bold text-stone-900 leading-[1.1] mb-8 serif ${isNepali ? 'lang-ne' : ''}`}>
            {t(UI_TEXT.heroTitle)}
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-5">
            ...
            >
              <span className={isNepali ? 'lang-ne' : ''}>{t(UI_TEXT.practiceTitle)}</span>
            ...
            >
              <span className={isNepali ? 'lang-ne' : ''}>{t(UI_TEXT.consultation)}</span>
            </a>
          </div>
          ...
          <div className="mt-16 flex flex-col sm:flex-row sm:items-center gap-6">
            ...
            <div>
              <p className={`text-xs text-stone-400 font-bold tracking-widest uppercase mb-1 ${isNepali ? 'lang-ne' : ''}`}>
